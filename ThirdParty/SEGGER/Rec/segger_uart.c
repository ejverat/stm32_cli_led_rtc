...
#define USART_TXE                                                              \
  7 // Transmit data register empty (Does NOT indicate that byte has already
    // been sent just indicates that it has been copied to the shift register)
...
static const U8 _abHelloMsg[_TARGET_HELLO_SIZE] = {
    ...
    (SEGGER_SYSVIEW_VERSION / 1000) %
        10}; // "Hello" message expected by SysView: [ 'S', 'V',
...
static void _cbOnUARTRx(U8 Data) {
  if (_SVInfo.NumBytesHelloRcvd <
      _SERVER_HELLO_SIZE) { // Not all bytes of <Hello> message received by
                            // SysView yet?
    _SVInfo.NumBytesHelloRcvd++;
    goto Done;
  }
  _StartSysView();
  SEGGER_RTT_WriteDownBuffer(_SVInfo.ChannelID, &Data,
                             1); // Write data into corresponding RTT buffer for
                                 // application to read and handle accordingly
Done:
  return;
}
...
static int _cbOnUARTTx(U8 *pChar) {
  ...
  if (_SVInfo.NumBytesHelloSent <
      _TARGET_HELLO_SIZE) { // Not all bytes of <Hello> message sent to SysView
                            // yet?
...
void HIF_UART_WaitForTxEnd(void) {
  ...
  while ((USART_SR & (1 << USART_TXE)) == 0)
    ; // Wait until transmit buffer empty (Last byte shift from data to shift
      // register)
...
void USART2_IRQHandler(void) {
  ...
  if (UsartStatus & (1 << USART_RXNE)) { // Data received?
    ...
    if ((UsartStatus & USART_RX_ERROR_FLAGS) ==
        0) {   // Only process data if no error occurred
  ...
  if (UsartStatus &
      (1 << USART_TXE)) { // Tx (data register) empty? => Send next character
                          // Note: Shift register may still hold a character
                          // that has not been sent yet.
    //
    // Under special circumstances, (old) BTL of Flasher does not wait until a
    // complete string has been sent via UART, so there might be an TxE
    // interrupt pending *before* the FW had a chance to set the callbacks
    // accordingly which would result in a NULL-pointer call... Therefore, we
    // need to check if the function pointer is valid.
    //
    ...
    if (r == 0) {                         // No more characters to send ?
      ...
      USART_SR; // Makes sure that "transmission complete" flag in USART_SR is
                // reset to 0 as soon as we write USART_DR. If USART_SR is not
                // read before, writing USART_DR does not clear "transmission
                // complete". See STM32F4 USART documentation for more detailed
                // description.
...
void HIF_UART_EnableTXEInterrupt(void) {
  USART_CR1 |= (1 << USART_TXEIE); // enable Tx empty interrupt => Triggered as
                                   // soon as data register content has been
...
void HIF_UART_Init(uint32_t Baudrate, UART_ON_TX_FUNC_P cbOnTx,
  ...
  v &= ~((15UL << ((GPIO_UART_TX_BIT) << 2)) |
         (15UL << ((GPIO_UART_RX_BIT) << 2)));
  v |=
      ((7UL << ((GPIO_UART_TX_BIT) << 2)) | (7UL << ((GPIO_UART_RX_BIT) << 2)));
  GPIO_AFRL = v;
  //
  ...
  v |= ((2UL << (GPIO_UART_TX_BIT << 1)) |
        (2UL << (GPIO_UART_RX_BIT << 1))); // PA10: alternate function
  GPIO_MODER = v;
  //
  // Initialize USART
  //
  USART_CR1 = 0 | (1 << 15) // OVER8  = 1; Oversampling by 8
  ...
  Div = ((2 * (UART_BASECLK)) / Div) +
        1; // Calculate divider for baudrate and round it correctly. This is
           // necessary to get a tolerance as small as possible.
  Div = Div / 2;
  if (Div > 0xFFF) {
  ...
  //
  // Setup callbacks which are called by ISR handler and enable interrupt in
  // NVIC
  //
  _cbOnRx = cbOnRx;
  _cbOnTx = cbOnTx;
  NVIC_SetPriority(USART_IRQn,
                   6); // Highest prio, so it is not disabled by embOS
